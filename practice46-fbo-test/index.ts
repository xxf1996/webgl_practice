(() => {
  ...
  const frameWidth = 300 // 帧宽
  const frameHeight = 200 // 帧高
  let half = {
    w: Math.floor(winW / 2),
    h: Math.floor(winH / 2)
  }
  ...
  function initData () {
    for (let i = 0; i < pointNum; i++) {
      let xIdx = i % (frameWidth - padding.x * 2) + padding.x
      let yIdx = Math.floor(i / (frameWidth - padding.x * 2)) + padding.y
      let xPos = Math.random()
      let yPos = Math.random() // 粒子坐标
      let size = Math.round(Math.random() * 10 + 2)
      // let tx = (xIdx * pixel.x) * 2 - 1 // 将帧缓冲顶点坐标范围变换到 [-1, 1] 
      // let ty = 1- (yIdx * pixel.y) * 2 // 同上
      let tx = xIdx / (frameWidth - 1) * 2 - 1 // 帧缓冲上面绘制时，点的位置范围是[-1, 1]
      let ty = yIdx / (frameHeight - 1) * 2 - 1
      textureVertices.push(tx, ty, 1)
      indices.push((xIdx + 0.5) / (frameWidth - 1), (yIdx + 0.5) / (frameHeight - 1), 1) // 从[0, 1]到[-1, 1]之间有两倍的差距，因此实际上帧缓冲写入的像素点之间有一像素的间隔，导致在取样的时候，取样点实际上写入像素点和空像素点之间的位置，取样后值就是原来的一半了！！！
