(function (global) {
  ...
  function addFace (info, p1, p2, p3, p4, face, colorInfo) {
    info.vertices.push(
      ...p3,
      ...p2,
      ...p1,
    ...
    let color = colorInfo || randomColor()
    let start = info.indices.length
    let curNormal = NomralList[face]
    for (let i = 0; i < 6; i++) {
      info.colors.push(...color)
      info.indices.push(start + i)
      info.normals.push(...curNormal)
    }
  }
  ...
  class Cube {
    constructor (gl, opts) {
      ...
      this._COLOR = opts.color || null // 指定颜色（六个面一致）
      ...
      this.viewMatrix = opts.view || mat4.create() // 视图矩阵
      this.modelMatrix = mat4.create() // 模型矩阵（旋转，平移，缩放等）
    ...
    initInfo () {
      ...
      if (this._COLOR) {
        ...
        addFace(this.info, p1, p2, p3, p4, 'top', color) // 上
        addFace(this.info, p6, p5, p8, p7, 'bottom', color) // 下
        addFace(this.info, p6, p2, p1, p5, 'left', color) // 左
        addFace(this.info, p8, p4, p3, p7, 'right', color) // 右
        addFace(this.info, p5, p1, p4, p8, 'front', color) // 前
        addFace(this.info, p7, p3, p2, p6, 'back', color) // 后
      } else {
        addFace(this.info, p1, p2, p3, p4, 'top') // 上
        addFace(this.info, p6, p5, p8, p7, 'bottom') // 下
        addFace(this.info, p6, p2, p1, p5, 'left') // 左
        addFace(this.info, p8, p4, p3, p7, 'right') // 右
        addFace(this.info, p5, p1, p4, p8, 'front') // 前
        addFace(this.info, p7, p3, p2, p6, 'back') // 后
      }
    }
    ...
    initVar () {
      ...
      for (let i = 0; i < uniformCount; i++) {
        ...
        if (uniformInfo.name === 'u_Matrix') {
          this.updateMatrix()
        } else if (uniformInfo.name === 'u_ModelMatrix') {
          this.updateModel()
        } else {
    ...
    initTexture (images) {
      ...
      let u_Box = gl.getUniformLocation(this.program, 'u_Box') // 找到纹理取样器属性定位
      gl.activeTexture(gl.TEXTURE0) // 开启0号纹理通道
      gl.uniform1i(u_Box, 0) // 纹理取样器赋值
      gl.bindTexture(gl.TEXTURE_CUBE_MAP, texture) // 绑定纹理对象到立方体纹理
      // 立方体纹理资源绑定
      ...
      gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_MIN_FILTER, gl.NEAREST) // 设置缩小取样参数
      gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_MAG_FILTER, gl.NEAREST) // 设置放大取样参数
      gl.texParameteri(gl.TEXTURE_CUBE_MAP, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE) // 设置两个面边缘取样参数
     ...
     */
    getModelMatrix () {
      mat4.mul(this.modelMatrix, this.rotateMatrix, this.translateMatrix)
      mat4.mul(this.modelMatrix, this.scaleMatrix, this.modelMatrix)
    ...
    getMVP () {
      let mvp = mat4.create()
      mat4.mul(mvp, this.viewMatrix, this.getModelMatrix())
    ...
    moveOrigin (pos) {
      let delta = [
        pos[0] - this._ORIGIN[0],
        pos[1] - this._ORIGIN[1],
    ...
    // 更新当前的模型矩阵属性
    updateModel () {
      this.updateUniform('u_ModelMatrix', this.getModelMatrix())
    }

    // 更新当前的mvp矩阵属性
    updateMatrix () {
      this.updateUniform('u_Matrix', this.getMVP())
    }
